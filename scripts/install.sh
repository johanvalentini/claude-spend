...
# Env:   CLAUDE_SPEND_PORT  port the collector listens on (default: ~/.config/claude-spend/config, else 4318)
#        FORCE=1            replace an OTEL endpoint that already points elsewhere
#
# The port is stored in ~/.config/claude-spend/config, which the collector reads at start,
# so the plist itself carries no settings. Re-running is safe: it re-syncs dependencies,
# reloads the agent and rewrites the env block.
set -euo pipefail
...
sed -e "s|__UV__|$UV|g" -e "s|__PROJECT__|$PROJECT|g" -e "s|__HOME__|$HOME|g" "$PLIST_SRC" > "$PLIST_DST"
...
# With CLAUDE_SPEND_PORT set, setup also stores the port in ~/.config/claude-spend/config.
uv run --project "$PROJECT" claude-spend setup ${PORT:+--port "$PORT"} ${FORCE:+--force}
PORT="$(uv run --project "$PROJECT" python -c 'import claude_spend; print(claude_spend.DEFAULT_PORT)')"
launchctl kickstart -k "gui/$(id -u)/$LABEL"   # pick up a port change
...
if [[ -z "$ok" ]]; then
    ...
    tail -n 20 "$HOME/Library/Logs/claude-spend-collector.log" "$HOME/Library/Logs/claude-spend-collector.stderr.log" >&2 || true
    exit 1
fi
...
Done. New Claude Code sessions will report to the collector (already-open ones will not).
  ...
  Port:       CLAUDE_SPEND_PORT=4319 $PROJECT/scripts/install.sh   (stored in ~/.config/claude-spend/config)
